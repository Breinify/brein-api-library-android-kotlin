...
object BreinCrypto {
    ...
    fun generateSignature(message: String?, secret: String?): String {
        ...
        return try {
            val e = secret?.toByteArray(Charset.forName("UTF-8"))
            val secretKey = SecretKeySpec(e, "HmacSHA256")
            val mac = Mac.getInstance("HmacSHA256")
            mac.init(secretKey)

            val hmacData: ByteArray =
                mac.doFinal(message?.toByteArray(Charset.forName("UTF-8")))

            val encodedString = hmacData.encodeBase64ToString()
            encodedString
